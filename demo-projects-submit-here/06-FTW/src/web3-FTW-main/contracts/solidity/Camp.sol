...
contract Camp {
    ...
    struct Participant {
        ...
        bool[] challengesCompleted; // 各关卡完成情况
    }
    ...
    CampFactory public factory;          // 工厂合约引用
    ...
    /**
     * @dev 内部函数：自动检查并更新营地状态
     */
    function _autoCheckCampState() internal {
        // 如果报名已截止且状态仍为Registration，则自动更新状态
        if (state == CampState.Registration && block.timestamp > signupDeadline) {
    ...
    constructor(
        ...
        factory = CampFactory(msg.sender);
    }
    ...
    function register() external payable inState(CampState.Registration) {
        ...
        // 通知工厂合约
        factory.registerParticipant(msg.sender);
    ...
    function configChallenges(
        ...
        // 确保营地状态为Success
        require(state == CampState.Success, "Camp is not in the required state");
     ...
     * @dev 开营失败时，参与者提取押金
     */
    function withdrawDepositOnFailure() external {
        // 自动检查并更新营地状态
        _autoCheckCampState();
        
        require(state == CampState.Failed, "Camp is not in failed state");
     ...
     */
    function batchRefundDeposits(address[] memory _participants) external onlyOrganizer {
        // 自动检查并更新营地状态
        _autoCheckCampState();
    ...
    function forfeitDeposits() external onlyOrganizer inState(CampState.Completed) {
        ...
        for (uint i = 0; i < participantCount; i++) {
            address payable participant = payable(address(uint160(i))); // 这里需要实际实现一个参与者地址列表
